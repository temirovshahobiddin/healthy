...
import CustomSelect from "~/shared/ui/custom-select.vue"
...
const getdata = async () => {
  return $http.$get("/specialists", { params: {} })
}
const { data } = await useAsyncData("specialists", () => getdata())

watchEffect(() => {
  specialists.value = (data.value?.data || []).map((item: any) => ({
    id: item.id,
    name: item.full_name
  }))
  slugs.value = (data.value?.data || []).map((item: any) => ({
    id: item.id,
    slug: item.slug
  }))
})
...
const addstep = async () => {
  if (currentStep.value == 1) {
    if (formState.specialist_id) {
      const slug = slugs.value.find((slug: any) => slug.id == formState.specialist_id)
      if (slug?.slug) {
        const response = await $http.$get(`specialists/${slug.slug}/services`)
        if (response?.data?.length > 0) {
          allServices.value = response.data.map((item: any) => ({
            id: item.id,
            name: item.title
          }))
        }
  ...
  <Transition name="modal">
    <div v-if="orderModalOpen"
      ...
      <div class="modal-content relative w-full rounded-t-[20px] bg-white px-4 pt-8 sm:w-fit sm:rounded-[20px] sm:p-6">
        ...
        <!-- Form -->
        <form v-if="!formSended" class="flex w-full max-w-[calc(100vw-32px)] flex-col gap-5 md:w-[710px]"
          @submit.prevent="submitForm">
          <!-- Steps -->
          <div class="relative w-full overflow-hidden rounded-xl bg-[#F8F8F8] px-4 py-2.5">
            <div class="text-sm font-bold text-green-500 sm:text-base">
              {{ t("order_modal.step") }} {{ currentStep }} {{ t("order_modal.of") }} {{ totalSteps }}
            </div>

            <!-- progress -->
            <div class="absolute bottom-0 left-0 h-[2px] w-full bg-green-500 transition-all duration-300"
              :style="{ width: `${(currentStep / totalSteps) * 100}%` }"></div>
          </div>

          <!-- Content -->
          <template v-if="currentStep === 1">
            <div class="text-2xl font-semibold sm:text-[30px]">{{ t("order_modal.select_specialist") }}</div>

            <CustomSelect v-model="formState.specialist_id" :placeholder="t('order_modal.search_specialist')"
              :options="specialists" :random-suggestions-count="4" />
          </template>

          <template v-else-if="currentStep === 2">
            <div class="text-2xl font-semibold sm:text-[30px]">{{ t("order_modal.select_service") }}</div>

            <CustomSelect v-model="formState.service_id" :placeholder="t('order_modal.search_service')"
              :options="allServices" :random-suggestions-count="4" />
          </template>

          <template v-else>
            <div class="text-2xl font-semibold sm:text-[30px]">{{ t("order_modal.personal_data") }}</div>
            <div class="grid w-full gap-5">
              <input v-model="formState.full_name"
                ...
                type="text" required :placeholder="t('order_modal.full_name')" />

              <div class="grid w-full gap-5 sm:grid-cols-2">
                <ui-mask-input v-model="formState.phone_number"
                  class="w-full border-b border-gray-300 bg-white py-3 text-base placeholder-gray-400 focus:border-green-500 focus:outline-none"
                  mask="+998 (##) ###-##-##" type="text" required :placeholder="t('order_modal.phone_number')"
                  :default-input="true" />

                <div class="relative w-full">
                  <input v-model="formState.meet_date"
                    ...
                    type="date" required :placeholder="t('order_modal.visit_date')" />
                  <div class="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 transform">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path
                        ...
                        stroke="#63845C" stroke-width="1.5" />
                      <path d="M7 4V2.5M17 4V2.5M2.5 9H21.5" stroke="#63845C" stroke-width="1.5"
              ...
              <textarea v-model="formState.comment"
                class="w-full border-b border-gray-300 bg-white py-3 text-base placeholder-gray-400 focus:border-green-500 focus:outline-none"
                rows="1" :placeholder="t('order_modal.comment')" />
            </div>
          </template>
          ...
          <div
            ...
            <div v-if="currentStep === totalSteps" class="flex items-start gap-3">
            ...
            <div class="flex w-full flex-col gap-3 sm:flex-row sm:gap-4">
              <ui-button v-if="currentStep != totalSteps" class="w-full sm:w-[210px]" type="button" @click="addstep">
                {{ t("order_modal.next") }}
              </ui-button>

              <ui-button v-else class="w-full sm:w-[210px]" type="submit" :loading>
                {{ t("order_modal.submit_request") }}
              </ui-button>

              <!-- <ui-button
                v-if="currentStep != 1 && currentStep != totalSteps"
                class="w-full !bg-[#F0F4F1] sm:w-[210px]"
                type="button"
                color="lime"
                variant="ghost"
                @click="currentStep++"
        ...
        <div v-else class="flex w-full flex-col items-center gap-2.5 text-center sm:max-w-[465px]">
          <i class="size-[70px] sm:size-[84px]">
            <svg class="h-full w-full" width="85" height="84" viewBox="0 0 85 84" fill="none"
          ...
          <div
            class="mt-5 flex w-[calc(100%+32px)] justify-center bg-[#F8F8F8] p-4 sm:w-full sm:flex-row sm:bg-transparent sm:p-0">
            <ui-button class="w-full sm:w-[230px]" type="submit" @click="
              () => {
                formSended = false
